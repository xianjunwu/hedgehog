  ...
  $.zui({
    ...
    clientLang: function () {
      ...
      if (!lang) {
        ...
        lang = hl ? hl : (navigator.userLanguage || navigator.userLanguage
            || 'zh_cn');
      }
...
(function ($) {
  ...
  $.fn.fixOlPd = function (pd) {
    ...
    return this.each(function () {
      var $ol = $(this);
      $ol.css('paddingLeft',
          Math.ceil(Math.log10($ol.children().length)) * pd + 10);
    });
  };
...
+function ($) {
  ...
  $(document).on('click.zui.button.data-api', '[data-toggle^=button]',
      function (e) {
        var $btn = $(e.target)
        if (!$btn.hasClass('btn')) {
          $btn = $btn.closest('.btn')
...
+function ($) {
  ...
  $.fn.alert = function (option) {
    return this.each(function () {
      ...
      if (!data) {
        $this.data(zuiname, (data = new Alert(this)))
      }
  ...
  $(document).on('click.' + zuiname + '.data-api', dismiss,
...
(function ($, undefined) {
  ...
  var Pager = function (element, options) {
    ...
    options = that.options = $.extend({}, Pager.DEFAULTS, this.$.data(),
        options);

    var lang = options.lang || $.zui.clientLang();
    that.lang = $.isPlainObject(lang) ? ($.extend(true, {},
        LANG[lang.lang || $.zui.clientLang()], lang)) : LANG[lang];
  ...
  Pager.prototype.set = function (page, recTotal, recPerPage, notTiggerChange) {
    ...
    state.totalPage = (state.recTotal && state.recPerPage) ? (Math.ceil(
        state.recTotal / state.recPerPage)) : 1;
    ...
    if (state.page && state.recTotal) {
      if (state.page < state.totalPage) {
        ...
        state.pageRecCount = state.recTotal - (state.recPerPage * (state.page
            - 1));
      }
    }
    ...
    that.state = state;
    if (!notTiggerChange && (oldState.page !== state.page || oldState.recTotal
        !== state.recTotal || oldState.recPerPage !== state.recPerPage)) {
  ...
  Pager.prototype.createLinkItem = function (page, text, asAElement) {
    ...
    var $ele = $('<a title="' + that.lang.pageOfText.format(page)
        + '" class="pager-item" data-page="' + page + '"/>').attr('href',
        page ? that.createLink(page, that.state) : '###').html(text);
    if (!asAElement) {
      $ele = $('<li />').append($ele).toggleClass('active',
          page === that.state.page).toggleClass('disabled',
  ...
  Pager.prototype.createNavItems = function (maxCount) {
    ...
    if (totalPage > 1) {
      if (totalPage <= maxCount) {
        ...
        appendItem(false);
        appendItem(page - Math.ceil((maxCount - 4) / 2),
            page + Math.floor((maxCount - 4) / 2));
        appendItem(false);
        appendItem(totalPage);
      }
    }
  ...
  Pager.prototype.createGoto = function () {
    ...
    var $goto = $(
        '<div class="input-group pager-goto" style="width: ' + (35 + (pager.page
        + '').length * 9 + 25 + that.lang.goto.length * 12)
        + 'px"><input value="' + pager.page + '" type="number" min="1" max="'
        + pager.totalPage + '" placeholder="' + pager.page
        + '" class="form-control pager-goto-input"><span class="input-group-btn"><button class="btn pager-goto-btn" type="button">'
        + that.lang.goto + '</button></span></div>');
    return $goto;
  };
  ...
  Pager.prototype.createSizeMenu = function () {
    ...
    for (var i = 0; i < options.length; ++i) {
      ...
      var $li = $('<li><a href="###" data-size="' + size + '">' + size
          + '</a></li>').toggleClass('active', size === pager.recPerPage);
      $menu.append($li);
    }
    return $(
        ...
        + ' <span class="caret"></span></button></div>').addClass(
        that.options.menuDirection).append($menu);
  };
    ...
    switch (element) {
      ...
      case 'prev_icon':
        return createLinkItem(pager.prev,
            '<i class="icon ' + that.options.prevIcon + '"></i>');
      case 'next':
        return createLinkItem(pager.next, lang.next);
      case 'next_icon':
        return createLinkItem(pager.next,
            '<i class="icon ' + that.options.nextIcon + '"></i>');
      case 'first':
        return createLinkItem(1, lang.first);
      case 'first_icon':
        return createLinkItem(1,
            '<i class="icon ' + that.options.firstIcon + '"></i>');
      case 'last':
        return createLinkItem(pager.totalPage, lang.last);
      case 'last_icon':
        return createLinkItem(pager.totalPage,
            '<i class="icon ' + that.options.lastIcon + '"></i>');
      case 'space':
      case '|':
      ...
      case 'total_text':
        return $(
            ('<div class="pager-label">' + lang.totalCount + '</div>').format(
                pager));
      case 'page_text':
        return $(('<div class="pager-label">' + lang.pageOf + '</div>').format(
            pager));
      case 'total_page_text':
        return $(
            ('<div class="pager-label">' + lang.totalPage + '</div>').format(
                pager));
      case 'page_of_total_text':
        return $(
            ('<div class="pager-label">' + lang.pageOfTotal + '</div>').format(
                pager));
      case 'page_size_text':
        return $(
            ('<div class="pager-label">' + lang.pageSize + '</div>').format(
                pager));
      case 'items_range_text':
        return $(
            ('<div class="pager-label">' + lang.itemsRange + '</div>').format(
                pager));
      case 'goto':
  ...
  Pager.prototype.render = function (elements) {
    ...
    for (var i = 0; i < elements.length; ++i) {
      ...
      var creator = isMapperCreator ? (createElement[element] || createElement)
          : createElement;
  ...
  $.fn.pager = function (option) {
    return this.each(function () {
      ...
      if (!data) {
        $this.data(NAME, (data = new Pager(this, options)));
      }
...
+function ($) {
  ...
  $.fn.tab = function (option) {
    return this.each(function () {
      ...
      if (!data) {
        $this.data(zuiname, (data = new Tab(this)))
      }
      if (typeof option == 'string') {
        data[option]()
      }
  ...
  $(document).on('click.zui.tab.data-api', '[data-toggle="tab"], [data-tab]',
      function (e) {
        e.preventDefault()
        $(this).tab('show')
      })
...
+function ($) {
  ...
  $.fn.collapse = function (option) {
    return this.each(function () {
      ...
      if (!data) {
        $this.data(zuiname, (data = new Collapse(this, options)))
      }
      if (typeof option == 'string') {
        data[option]()
      }
  ...
  $(document).on('click.' + zuiname + '.data-api', '[data-toggle=collapse]',
      function (e) {
        var $this = $(this),
            href
        ...
        if (!data || !data.transitioning) {
          if ($parent) {
            $parent.find(
                '[data-toggle=collapse][data-parent="' + parent + '"]').not(
...
(function (window, $) {
  ...
  var resetCssClass = function () {
    ...
    $('html').toggleClass('screen-desktop',
        width >= desktop && width < desktopLg)
...
(function ($) {
  ...
  Browser.prototype.tip = function (showCoontent) {
    ...
    $browseHappy.find('.content').html(
        showCoontent || this.browseHappyTip || browseHappyTip[$.zui.clientLang()
        || 'zh_cn']);
  };
        ...
        if (arguments.length <= 2 && typeof (args) == 'object') {
          for (var key in args) {
            if (args[key] !== undefined) {
              reg = new RegExp(
                  '(' + (arguments[1] ? arguments[1].replace('0', key) : '{'
                  + key + '}') + ')', 'g');
...
+function ($) {
  ...
  function ScrollSpy(element, options) {
    ...
    this.$body = $('body')
    this.$scrollElement = this.$element.on('scroll.' + zuiname + '.data-api',
        process)
    this.options = $.extend({}, ScrollSpy.DEFAULTS, options)
    if (!this.selector) {
      this.selector = (this.options.target || ((href = $(
          element).attr('href')) && href.replace(/.*(?=#[^\s]+$)/, '')) //strip for ie7
          || '') + ' .nav li > a'
    }
    this.offsets = $([])
    this.targets = $([])
    ...
    .map(function () {
      ...
      return ($href && $href.length && $href.is(':visible') && [
        [$href[offsetMethod]().top + (!$.isWindow(self.$scrollElement.get(0))
            && self.$scrollElement.scrollTop()), href]
      ]) || null
    })
    .sort(function (a, b) {
      return a[0] - b[0]
  ...
  ScrollSpy.prototype.process = function () {
    ...
    for (i = offsets.length; i--;) {
      activeTarget != targets[i] && scrollTop >= offsets[i] && (!offsets[i + 1]
  ...
  $.fn.scrollspy = function (option) {
    return this.each(function () {
      ...
      if (!data) {
        $this.data(zuiname, (data = new ScrollSpy(this, options)))
      }
      if (typeof option == 'string') {
        data[option]()
      }
...
(function (window, $) {
  ...
  Store.prototype.pageGet = function (key, defaultValue) {
    ...
    return (defaultValue !== undefined && (val === null || val === undefined))
        ? defaultValue : val;
  };
...
(function ($) {
  ...
  var SearchBox = function (element, options) {
    ...
    that.options = options = $.extend({}, SearchBox.DEFAULTS, that.$.data(),
        options);

    // Initialize here
    var $input = that.$.is(options.inputSelector) ? that.$ : that.$.find(
  ...
  $.fn.searchBox = function (option) {
    return this.each(function () {
      ...
      if (!data) {
        $this.data(NAME, (data = new SearchBox(this, options)));
      }
...
(function ($, document) {
  ...
  $.fn.draggable = function (option) {
    return this.each(function () {
      ...
      if (!data) {
        $this.data(NAME, (data = new Draggable(this, options)));
      }
      if (typeof option == 'string') {
        data[option]();
      }
...
(function ($, document, Math) {
  ...
  Droppable.prototype.init = function () {
    var that = this,
        ...
        $ele = $root,
        isMouseDown = false,
        $container = container ? $(setting.container).first() : (selector
            ? $root : $('body')),
        $targets,
        $target,
        $shadow,
    ...
    var mouseMove = function (event) {
      ...
      if (Math.abs(mouseOffset.left - startMouseOffset.left) < deviation
          && Math.abs(mouseOffset.top - startMouseOffset.top)
          < deviation) {
        return;
      }
      ...
      if ($shadow === null) // create shadow
      {
        ...
        $shadow = $ele.clone().removeClass('drag-from').addClass(
            'drag-shadow').css({
          position: 'absolute',
      ...
      $targets.each(function () {
        ...
        if (mouseOffset.left > tX && mouseOffset.top > tY && mouseOffset.left
            < (tX + tW) && mouseOffset.top < (tY + tH)) {
          if ($newTarget) {
            $newTarget.removeClass(dropToClass);
          }
          $newTarget = t;
          if (!setting.nested) {
            return false;
      ...
      if ($newTarget) {
        ...
        if ($target === null || ($target.data('id') !== id
            && (!isSelf))) {
          isNew = true;
        }
        $target = $newTarget;
    ...
    var mouseDown = function (event) {
      ...
      isMouseDown = true;
      $targets = $.isFunction(setting.target) ? setting.target($ele, $root)
          : $container.find(setting.target),
          $target = null,
          $shadow = null,
          isIn = false,
          isSelf = true,
  ...
  $.fn.droppable = function (option) {
    return this.each(function () {
      ...
      if (!data) {
        $this.data(NAME, (data = new Droppable(this, options)));
      }
...
+function ($, undefined) {
  ...
  var setDialogPos = function ($dialog, pos) {
    var $window = $(window);
    pos.left = Math.max(0,
        Math.min(pos.left, $window.width() - $dialog.outerWidth()));
    pos.top = Math.max(0,
        Math.min(pos.top, $window.height() - $dialog.outerHeight()));
    $dialog.css(pos);
  };
  ...
  Modal.prototype.setMoveale = function () {
    ...
    var that = this;
    var options = that.options;
    var $dialog = that.$element.find('.modal-dialog').removeClass(
        'modal-dragged');
      ...
      $dialog.draggable({
        ...
        before: function () {
          ...
          if (marginTop && marginTop !== '0px') {
            $dialog.css('top', marginTop).css('margin-top', '').addClass(
                'modal-dragged');
          }
        },
        finish: function (e) {
          ...
          if (rememberPos) {
            ...
            if ($.zui.store && rememberPos !== true) {
              $.zui.store.pageSet(zuiname + '.rememberPos.' + rememberPos,
                  e.pos);
            }
          }
        },
  ...
  Modal.prototype.show = function (_relatedTarget, position) {
    ...
    that.$element.toggleClass('modal-scroll-inside',
        !!that.options.scrollInside);
    ...
    that.$element.on('click.dismiss.' + zuiname, '[data-dismiss="modal"]',
        function (e) {
          that.hide();
          e.stopPropagation();
        })
  ...
  Modal.prototype.enforceFocus = function () {
    ...
    .on('focusin.' + zuiname, $.proxy(function (e) {
      if (this.$element[0] !== e.target && !this.$element.has(
          e.target).length) {
  ...
  function Plugin(option, _relatedTarget, position) {
    return this.each(function () {
      ...
      if (!data) {
        $this.data(zuiname, (data = new Modal(this, options)))
      }
      if (typeof option == 'string') {
        data[option](_relatedTarget, position)
      } else if (options.show) {
        data.show(_relatedTarget, position)
      }
    })
  }
  ...
  $(document).on('click.' + zuiname + '.data-api', '[data-toggle="modal"]',
      function (e) {
        var $this = $(this)
        ...
        try {
          // strip for ie7
          $target = $($this.attr('data-target') || (href && href.replace(
              /.*(?=#[^\s]+$)/, '')));
        } catch (ex) {
          return
        }
...
(function ($, window, undefined) {
  ...
  var ModalTrigger = function (options, $trigger) {
    options = $.extend({}, ModalTrigger.DEFAULTS, $.ModalTriggerDefaults,
        $trigger ? $trigger.data() : null, options);
    this.isShown;
  ...
  ModalTrigger.prototype.init = function (options) {
    var that = this;
    if (options.url) {
      if (!options.type || (options.type != STR_AJAX && options.type
          != 'iframe')) {
    ...
    if (options.remote) {
      options.type = STR_AJAX;
      if (typeof options.remote === STR_STRING) {
        options.url = options.remote;
      }
    } else if (options.iframe) {
      options.type = 'iframe';
      if (typeof options.iframe === STR_STRING) {
        options.url = options.iframe;
      }
    ...
    $modal = $('<div id="' + options.name + '" class="modal modal-trigger '
        + (options.className || '') + '">' + (typeof options.loadingIcon
        === 'string' && options.loadingIcon.indexOf('icon-') === 0
            ? ('<div class="icon icon-spin loader ' + options.loadingIcon
                + '"></div>') : options.loadingIcon)
        + '<div class="modal-dialog"><div class="modal-content"><div class="modal-header"><button class="close" data-dismiss="modal">×</button><h4 class="modal-title"><i class="modal-icon"></i> <span class="modal-title-name"></span></h4></div><div class="modal-body"></div></div></div></div>').appendTo(
        'body').data(NAME, that);

    var bindEvent = function (optonName, eventName) {
      var handleFunc = options[optonName];
      if ($.isFunction(handleFunc)) {
        $modal.on(eventName + ZUI_MODAL,
            handleFunc);
      }
    };
  ...
  ModalTrigger.prototype.show = function (option) {
    var options = $.extend({}, this.options, {
      url: this.$trigger ? (this.$trigger.attr('href') || this.$trigger.attr(
          'data-url') || this.$trigger.data('url')) : this.options.url
    }, option);
    ...
    $header.find('.modal-icon').attr('class',
        'modal-icon icon-' + options.icon);
      ...
      return setTimeout(function () {
        ...
        if (options.height && options.height != 'auto') {
          ...
          if (options.type === 'iframe') {
            $body.css('height',
                $dialog.height() - $header.outerHeight());
          }
        }
        that.ajustPosition(options.position);
    ...
    if (options.type === 'custom' && custom) {
      ...
      if (options.type === 'iframe') {
        ...
        $body.css('padding', 0)
        .html('<iframe id="' + iframeName + '" name="' + iframeName + '" src="'
            + options.url
        ...
        frame.onload = frame.onreadystatechange = function () {
          ...
          try {
            ...
            if (frame$ && options.height === 'auto' && options.size
                != 'fullscreen') {
              // todo: update iframe url to ref attribute

              var $framebody = frame$('body').addClass(
                  'body-modal').toggleClass('body-modal-scroll-inside',
                  scrollInside);
              if (options.iframeBodyClass) {
                $framebody.addClass(
                    options.iframeBodyClass);
              }
              ...
              var ajustFrameSize = function (check) {
                ...
                if (frameSizeRecords.length > 1 && height
                    === frameSizeRecords[0]) {
              ...
              $framebody.off('resize.' + NAME).on('resize.' + NAME,
                  ajustFrameSize);
              if (scrollInside) {
                $(window).off('resize.' + NAME).on('resize.' + NAME,
                    ajustFrameSize);
              }
            } else {
              readyToShow();
          ...
          success: function (data) {
            try {
              ...
              if (window.console && window.console.warn) {
                console.warn('ZUI: Cannot recogernize remote content.',
                    {error: e, data: data});
              }
              $modal.html(data);
            }
  ...
  ModalTrigger.prototype.close = function (callback, redirect) {
    ...
    if (callback || redirect) {
      that.$modal.on('hidden' + ZUI_MODAL, function () {
        ...
        if (typeof redirect === STR_STRING && redirect.length
            && !that.$modal.data('cancel-reload')) {
          if (redirect === 'this') {
  ...
  $.fn.modalTrigger = function (option, settings) {
    return $(this).each(function () {
      ...
      if (!data) {
        $this.data(NAME, (data = new ModalTrigger(options, $this)));
      }
      if (typeof option == STR_STRING) {
      ...
      $this.on((options.trigger || 'click') + '.toggle.' + NAME, function (e) {
        options = $.extend(options, {
          url: $this.attr('href') || $this.attr('data-url') || $this.data('url')
              || options.url
        });
        data.toggle(options);
        if ($this.is('a')) {
  ...
  $.fn.modal = function (option, settings) {
    return $(this).each(function () {
      ...
      if ($this.hasClass('modal')) {
        old.call($this, option, settings);
      } else {
        $this.modalTrigger(option, settings);
      }
    });
  };
  $.fn.modal.bs = old;
  ...
  $(document).on('click.' + NAME + '.data-api', '[data-toggle="modal"]',
      function (e) {
        var $this = $(this);
        var href = $this.attr('href');
        var $target = null;
      ...
      }).on('click.' + NAME + '.data-api', '[data-dismiss="modal"]',
      function () {
        $.zui.closeModal();
      });
...
+function ($) {
  ...
  Tooltip.prototype.show = function (content) {
    ...
    if ((content || this.hasContent()) && this.enabled) {
      ...
      that.options.container ? $tip.appendTo(that.options.container)
          : $tip.insertAfter(that.$element)
      ...
      if (autoPlace) {
        ...
        var parentWidth = that.options.container == 'body' ? window.innerWidth
            : $parent.outerWidth()
        var parentHeight = that.options.container == 'body' ? window.innerHeight
            : $parent.outerHeight()
        var parentLeft = that.options.container == 'body' ? 0
            : $parent.offset().left
      ...
      var calculatedOffset = that.getCalculatedOffset(placement, pos,
  ...
  Tooltip.prototype.toggle = function (e) {
    var self = e ? $(e.currentTarget)[this.type](
        this.getDelegateOptions()).data('zui.' + this.type) : this
  ...
  $.fn.tooltip = function (option, params) {
    return this.each(function () {
      ...
      if (!data) {
        $this.data('zui.tooltip', (data = new Tooltip(this, options)))
      }
      if (typeof option == 'string') {
        data[option](params)
      }
...
+function ($) {
  ...
  Popover.prototype.getTarget = function () {
    ...
    var target = $e.attr('data-target') || (typeof o.target == 'function' ?
        o.target.call($e[0]) :
        o.target)
    return (target && true) ? (target == '$next' ? $e.next('.popover') : $(
        target)) : false
  }
  ...
  $.fn.popover = function (option) {
    return this.each(function () {
      ...
      if (!data) {
        $this.data('zui.popover', (data = new Popover(this, options)))
      }
      if (typeof option == 'string') {
        data[option]()
      }
...
+function ($) {
  ...
  function getParent($this) {
    ...
    if (!selector) {
      selector = $this.attr('href')
      selector = selector && /#/.test(selector) && selector.replace(
          /.*(?=#[^\s]*$)/, '') //strip for ie7
    }
    var $parent;
    try {
  ...
  .on('keydown.' + apiName, toggle + ', [role=menu]',
...
(function ($, undefined) {
  ...
  var createMenuItem = function (item, index) {
    ...
    return $('<li />').toggleClass('disabled', item.disabled === true).append(
        $a);
  };
  ...
  var showContextMenu = function (items, options, callback) {
    ...
    var y = options.y;
    if (x === undefined) {
      x = (options.event || options).clientX;
    }
    if (x === undefined) {
      x = mouseX;
    }
    if (y === undefined) {
      y = (options.event || options).clientY;
    }
    if (y === undefined) {
      y = mouseY;
    }
    ...
    if (!$target.length) {
      $target = $(
          '<div style="display: none; position: fixed; z-index: 2000;" class="contextmenu" id="'
          + targetId
          + '"><ul class="dropdown-menu contextmenu-menu"></ul></div>').appendTo(
          'body');
    }
    var $menu = $target.find('.contextmenu-menu').off('click.' + NAME).on(
        'click.' + NAME, 'a', function (e) {
          var $item = $(this);
          var clickResult = options.onClickItem && options.onClickItem(
              $item.data('item'), $item, e);
    ...
    if (animation === true) {
      options.animation = animation = 'fade';
    }
    if (animationTimer) {
      clearTimeout(animationTimer);
      animationTimer = null;
  ...
  var ContextListener = function (element, options) {
    ...
    options = that.options = $.extend({trigger: 'contextmenu'},
        ContextMenu.DEFAULTS, this.$.data(), options);
  ...
  $.fn.contextmenu = function (option) {
    return this.each(function () {
      ...
      if (!data) {
        $this.data(NAME, (data = new ContextListener(this, options)));
      }
...
+function ($) {
  ...
  Carousel.prototype.cycle = function (e) {
    ...
    this.options.interval && !this.paused && (this.interval = setInterval(
  ...
  $(document).on('click.zui.carousel.data-api', '[data-slide], [data-slide-to]',
      function (e) {
        var $this = $(this),
            href
...
(function ($, window, Math) {
  ...
  Lightbox.prototype.getOptions = function (options) {
    ...
    if (!options[IMAGE]) {
      options[IMAGE] = this.$.attr('src') || this.$.attr('href') || this.$.find(
          'img').attr('src');
  ...
  Lightbox.prototype.initGroups = function () {
    ...
    if (!groups) {
      groups = $('[data-toggle="lightbox"][data-group="' + this.options.group
          + '"], [data-lightbox-group="' + this.options.group + '"]');
  ...
  Lightbox.prototype.bindEvents = function () {
    ...
    $e.modalTrigger({
      ...
      custom: function (e) {
        ...
        .toggleClass('lightbox-with-caption',
        ...
        $.zui.imgReady(options.image, function () {
          ...
          if (winWidth < (this.width + 30)) {
            modal.addClass('lightbox-full');
          }
          e.ready(200);
        });

        modal.find('.prev').toggleClass('show', groups.filter(
            '[data-group-index="' + (groupIndex - 1) + '"]').length > 0);
        modal.find('.next').toggleClass('show', groups.filter(
            '[data-group-index="' + (groupIndex + 1) + '"]').length > 0);

        modal.find('.controller').click(function () {
          var $this = $(this);
          var id = modal.data('group-index') + ($this.hasClass('prev') ? -1
              : 1);
          ...
          modal.find('.prev').toggleClass('show',
              groups.filter('[data-group-index="' + (id - 1) + '"]').length
              > 0);
  ...
  $.fn.lightbox = function (option) {
    ...
    return this.each(function () {
      ...
      if (!data) {
        $this.data('zui.lightbox',
            (data = new Lightbox(this, options)));
      }
...
(function ($, window, undefined) {
  ...
  var Messager = function (message, options) {
    ...
    all[that.id] = that;
    that.message = (options.icon ? '<i class="icon-' + options.icon
        + ' icon"></i> ' : '') + message;

    that.$ = $(template.format(options)).toggleClass('fade',
        options.fade).toggleClass('scale', options.scale).attr('id',
        'messager-' + that.id);

    if (options.cssClass) {
      that.$.addClass(options.cssClass);
    }
    ...
    var appendAction = function (action) {
      var $btn = $(
          '<button type="button" class="action action-' + action.name + '"/>');
  ...
  Messager.prototype.show = function (message, callback) {
    ...
    if (!$holder.length) {
      $holder = $('<div/>').attr('class',
          'messagers-holder ' + placement).appendTo($parent);
    }
    $holder.append(that.$);
    if (placement === 'center') {
...
(function ($, Math, window, undefined) {
    ...
    if (isNotUndefined(r) && isNotUndefined(g) && isNotUndefined(b)) {
      ...
      if (type == STR_STRING) {
        ...
        if (r === 'transparent') {
          ...
          var rgbsArr = r.substring(r.indexOf('(') + 1,
              r.lastIndexOf(')')).split(',', 4);
          that.rgb({
            r: rgbsArr[0],
            g: rgbsArr[1],
            b: rgbsArr[2],
        ...
        if (isNotUndefined(r.g)) {
          that.g = convertToRgbInt(r.g);
        }
        if (isNotUndefined(r.b)) {
          that.b = convertToRgbInt(r.b);
        }
        if (isNotUndefined(r.a)) {
          that.a = clamp(number(r.a), 1);
        }
        ...
        if (isNotUndefined(r.s)) {
          hsl.s = clamp(number(r.s), 1);
        }
        if (isNotUndefined(r.l)) {
          hsl.l = clamp(number(r.l), 1);
        }
        if (isNotUndefined(r.a)) {
          hsl.a = clamp(number(r.a), 1);
        }
  ...
  Color.prototype.rgb = function (rgb) {
    ...
    if (isNotUndefined(rgb)) {
      if (typeof (rgb) == STR_OBJECT) {
        if (isNotUndefined(rgb.r)) {
          that.r = convertToRgbInt(rgb.r);
        }
        if (isNotUndefined(rgb.g)) {
          that.g = convertToRgbInt(rgb.g);
        }
        if (isNotUndefined(rgb.b)) {
          that.b = convertToRgbInt(rgb.b);
        }
        if (isNotUndefined(rgb.a)) {
          that.a = clamp(number(rgb.a), 1);
        }
      } else {
  ...
  Color.prototype.contrast = function (dark, light, threshold) {
    if (isUndefined(light)) {
      light = new Color(N255, N255, N255, 1);
    } else {
  ...
  function isColor(hex) {
    return typeof (hex) === STR_STRING && (hex.toLowerCase() === 'transparent'
        || namedColors[hex.toLowerCase()] || hexReg.test(
            $.trim(hex.toLowerCase())));
  }
...
(function ($) {
  ...
  function formatActions(actions, parentActions) {
    ...
    if ($.isPlainObject(actions)) {
      ...
      $.each(actions, function (name, action) {
        if (action) {
          _actions[name] = $.extend({type: name}, DETAULT_ACTIONS[name],
  ...
  function createActionEle(action, name, template) {
    ...
    return $(template || action.template).addClass('tree-action').attr(
        $.extend({
          'data-type': name,
          title: action.title || ''
  ...
  Tree.prototype.add = function (rootEle, items, expand, disabledAnimate,
      notStore) {
    ...
    if ($ul) {
      ...
      $.each(items, function (idx, item) {
        ...
        var $wrapper = options.itemWrapper ? $(
            options.itemWrapper === true ? '<div class="tree-item-wrapper"/>'
                : options.itemWrapper).appendTo($li) : $li;
        if (item.html) {
    ...
    if (that.isPreserve) {
      if (that.store.time) {
        that.$.find('li:not(.tree-action-item)').each(function () {
          var $li = $(this);
          that[that.store[$li.data('id')] ? 'expand' : 'collapse']($li, true,
              true);
        });
      }
    }
  ...
  Tree.prototype._initList = function ($list, $parentItem, idx, data) {
    ...
    var $children = $list.attr('data-idx', idx || 0).children(
        'li:not(.tree-action-item)').each(function (index) {
    ...
    if (actions) {
      if (actions.add && actions.add.templateInList !== false) {
        ...
        if (!$actionItem.length) {
          $('<li class="tree-action-item"/>').append(
              createActionEle(actions.add, 'add',
                  actions.add.templateInList)).appendTo($list);
        } else {
      ...
      if (actions.sort) {
        $list.sortable($.extend({
            ...
            $.isPlainObject(this.options.sortable) ? this.options.sortable
                : null));
      }
    }
    ...
    if (actions) {
      ...
      if (!$actions.length) {
        $actions = $('<div class="tree-actions"/>').appendTo(
            this.options.itemWrapper ? $item.find('.tree-item-wrapper')
                : $item);
  ...
  Tree.prototype._init = function () {
    ...
    if (isPreserveEnable) {
      this.selector = name + '::' + (options.name || '') + '#' + (this.$.attr(
          'id') || globalId++);
    ...
    if (initialState === 'preserve') {
      if (isPreserveEnable) {
        ...
        this.options.initialState = initialState = 'normal';
      }
    }
    ...
    this.$.on('click', '.list-toggle,a[href="#"],.tree-toggle', function (e) {
      ...
      that.callEvent('action',
          {action: action, target: this, $item: $li, item: $li.data()});
    });
  };
  ...
  Tree.prototype.preserve = function ($li, id, expand) {
    ...
    if ($li) {
      ...
      if (expand) {
        this.store[id] = expand;
      } else {
        delete this.store[id];
      }
      this.store.time = new Date().getTime();
      $.zui.store[this.options.name ? 'set' : 'pageSet'](this.selector,
          this.store);
    } else {
      var that = this;
      this.store = {};
  ...
  Tree.prototype.toggle = function ($li) {
    var collapse = ($li && $li.hasClass('open')) || $li === false || ($li
        === undefined && this.$.find('li.has-list.open').length);
  ...
  Tree.prototype.toData = function ($ul, filter) {
    ...
    return $ul.children('li:not(.tree-action-item)').map(function () {
      ...
      if ($children.length) {
        data.children = that.toData($children);
      }
  ...
  $.fn.tree = function (option, params) {
    return this.each(function () {
      ...
      if (!data) {
        $this.data(name, (data = new Tree(this, options)));
      }
