...
(function ($, Math, undefined) {
  ...
  if (!$.fn.draggable) {
    console.error('img-cutter requires draggable.js');
  }
  if (!$.zui.imgReady) {
    console.error('img-cutter requires image.ready.js');
  }
  ...
  ImgCutter.prototype.refreshSize = function (ratioSide) {
    ...
    this.clipWidth = Math.max(options.minWidth,
        Math.min(this.width, this.clipWidth));
    this.clipHeight = Math.max(options.minHeight,
        Math.min(this.height, this.clipHeight));

    if (options.fixedRatio) {
      if (ratioSide && ratioSide === 'height') {
        this.clipWidth = Math.max(options.minWidth, Math.min(this.width,
            this.clipHeight * options.defaultWidth / options.defaultHeight));
        this.clipHeight = this.clipWidth * options.defaultHeight
            / options.defaultWidth;
      } else {
        this.clipHeight = Math.max(options.minHeight, Math.min(this.height,
            this.clipWidth * options.defaultHeight / options.defaultWidth));
        this.clipWidth = this.clipHeight * options.defaultWidth
    ...
    this.$cliper.css('clip', 'rect({0}px {1}px {2}px {3}px'.format(this.top,
        this.left + this.clipWidth, this.top + this.clipHeight, this.left));
    ...
    this.$controller.children('.control').draggable({
      ...
      drag: function (e) {
        ...
        switch (dr) {
          ...
          case 'bottom-left':
            that.left += offset.x;
            that.left = Math.min(that.right - options.minWidth,
                Math.max(0, that.left));
          ...
          case 'bottom-right':
            ...
            that.clipWidth = Math.min(that.width - that.left,
                Math.max(options.minWidth, that.clipWidth));
            break;
        }
        switch (dr) {
          case 'top':
          ...
          case 'top-right':
            that.top += offset.y;
            that.top = Math.min(that.bottom - options.minHeight,
                Math.max(0, that.top));
          ...
          case 'bottom-right':
            ...
            that.clipHeight = Math.min(that.height - that.top,
                Math.max(options.minHeight, that.clipHeight));
            ratioSide = true;
            break;
        }
  ...
  $.fn.imgCutter = function (option) {
    return this.each(function () {
      ...
      if (!data) {
        $this.data(NAME, (data = new ImgCutter(this, options)));
      }
