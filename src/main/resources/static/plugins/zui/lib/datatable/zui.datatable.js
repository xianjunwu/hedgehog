...
(function ($) {
  ...
  DataTable.prototype.getOptions = function (options) {
    ...
    $.each(['bordered', 'condensed', 'striped', 'condensed', 'fixed'],
        function (idx, cls) {
          cls = 'table-' + cls;
          if ($e.hasClass(cls)) {
    ...
    if (!options.checkable || !$.fn.selectable) {
  ...
  DataTable.prototype.load = function (data) {
    ...
    if (!data) {
      if (this.isTable) {
        ...
        $t.children('thead').children('tr:first').children('th').each(
            function () {
              $th = $(this);
              cols.push($.extend({
                text: $th.html(),
        ...
        if ($tfoot.length) {
          data.footer = $(
              '<table class="table' + options.tableClass + '"></table>').append(
              $tfoot);
        }
      } else {
  ...
  DataTable.prototype.render = function () {
    ...
    var $datatable = that.$datatable || (that.isTable ? $(
        '<div class="datatable" id="' + that.id + '"/>') : that.$datatable),
    ...
    $datatable.children(
        '.datatable-head, .datatable-rows, .scroll-wrapper').remove();
    ...
    for (i = 0; i < cols.length; i++) {
      col = cols[i];
      $tr = i < data.flexStart ? $left : ((i >= data.flexStart && i
          <= data.flexEnd) ? $flex : $right);
      if (i === 0 && checkable) {
        $tr.append(
    ...
    for (var r = 0; r < rowLen; ++r) {
      ...
      for (i = 0; i < rowColLen; ++i) {
        ...
        $tr = i < data.flexStart ? $leftRow : ((i >= data.flexStart && i
            <= data.flexEnd) ? $flexRow : $rightRow);
        if (i === 0 && checkable) {
          $cTd = $(
              '<td data-index="check" class="check-row check-btn"><i class="icon-check-empty"></i></td>');
          if (options.checkboxName) {
            $cTd.append('<input class="hide" type="checkbox" name="'
                + options.checkboxName + '" value="' + row.id + '">');
          }
          $tr.append($cTd);
        }
    ...
    if (data.flexArea) {
      $datatable.append(
          ...
          + options.scrollPos + '"><div class="bar"></div></div></div>');
    }

    var $oldFooter = $datatable.children('.datatable-footer').detach();
    if (data.footer) {
      $datatable.append(
          $('<div class="datatable-footer"/>').append(data.footer));
      data.footer = null;
  ...
  DataTable.prototype.bindEvents = function () {
    ...
    var $dataSpans = that.$dataSpans = $datatable.children(
        '.datatable-head, .datatable-rows').find('.datatable-span');
    var $rowsSpans = that.$rowsSpans = $datatable.children(
        '.datatable-rows').children('.datatable-rows-span');
    var $headSpans = that.$headSpans = $datatable.children(
        '.datatable-head').children('.datatable-head-span');
    var $cells = that.$cells = $dataSpans.find(
        '.datatable-head-cell,.datatable-cell');
    ...
    if (options.rowHover) {
      ...
      $rowsSpans.on('mouseenter', '.datatable-cell', function () {
        ...
        $rows.filter('[data-index="' + $(this).addClass(hoverClass).data('row')
            + '"]').addClass(hoverClass);
    ...
    if (options.colHover) {
      ...
      $headSpans.on('mouseenter', '.datatable-head-cell', function () {
        ...
        $cells.filter('[data-index="' + $(this).data('index') + '"]').addClass(
            colHoverClass);
    ...
    if (data.flexArea) {
      var $scrollbar = $datatable.find('.scroll-slide'),
          ...
          $flexTable = $datatable.find(
              '.datatable-span.flexarea .table-datatable');
      ...
      var srollTable = function (offset, silence) {
        ...
        left = 0 - Math.floor(
            (tableWidth - flexWidth) * barLeft / (flexWidth - scrollWidth));
      ...
      var dragOptions = {
        ...
        drag: function (e) {
          srollTable(
              $bar.position().left + e.smallOffset.x * (e.element.hasClass(
              'bar') ? 1 : -1));
        },
        finish: function () {
      ...
      if ($.fn.draggable) {
        ...
        if (options.flexHeadDrag) {
          $datatable.find('.datatable-head-span.flexarea').draggable(
              dragOptions);
        }
      } else {
    ...
    if (options.checkable) {
      ...
      var syncChecks = function () {
        ...
        if (options.checkboxName) {
          $checkRows.find(
              '.check-row input:checkbox').prop('checked', false);
        }
        var checkedStatus = {
          checkedAll: $checkRows.length === $checkedRows.length
        ...
        $headSpans.find('.check-all').toggleClass('checked',
            !!checkedStatus.checkedAll);

        if (options.storage) {
          store.pageSet(checkedStatusStoreName,
              checkedStatus);
        }
      ...
      var toggleRowClass = function (ele, toggle) {
        ...
        $rows.filter('[data-index="' + $tr.data('index') + '"]').toggleClass(
            checkedClass, !!toggle);
      };
      ...
      if (options.selectable) {
        var selectableOptions = {
          ...
          rangeFunc: function (range, targetRange) {
            return Math.max(range.top, targetRange.top) < Math.min(
                range.top + range.height, targetRange.top + targetRange.height);
          },
          select: function (e) {
        ...
        this.$rowsSpans.off(checkEventPrefix).on(checkEventPrefix + 'row',
            options.checkByClickRow ? 'tr' : '.check-row', function () {
              toggleRowClass(this);
              syncChecks();
            });
      }

      this.$datatable.off(checkEventPrefix).on('click.zui.datatable.check',
          '.check-all', function () {
            $rows.toggleClass(checkedClass,
                $(this).toggleClass('checked').hasClass('checked'));
            syncChecks();
    ...
    if (options.fixedHeader) {
      var offsetTop,
          ...
          navbarHeight = options.fixedHeaderOffset || $(
              '.navbar.navbar-fixed-top').height() || 0;
      var handleScroll = function () {
        offsetTop = $datatable.offset().top;
        scrollTop = $(window).scrollTop();
        height = $datatable.height();
        $datatable.toggleClass('head-fixed',
            (scrollTop + navbarHeight) > offsetTop && (scrollTop + navbarHeight)
            < (offsetTop + height));
          ...
          $cs.each(function () {
            ...
            if ($lastCell) {
              if ($cell.html() === $lastCell.html()) {
                ...
                $lastCell.attr('rowspan', rowspan + 1).css('vertical-align',
                    'middle');
                $cell.remove();
              } else {
                $lastCell = $cell;
  ...
  DataTable.prototype.sortTable = function ($th) {
    ...
    if (!$th) {
      if (sorter) {
        $th = this.$headCells.filter(
            '[data-index="' + sorter.index + '"]').addClass(
            'sort-' + (sorter.type === 'up' ? 'down' : 'up'));
      } else {
    ...
    var valA, valB, result,
        $dataRows = this.$dataCells.filter('[data-index="' + index + '"]');
  ...
  DataTable.prototype.refreshSize = function () {
    ...
    $datatable.find('.datatable-span.fixed-left').css('width',
        options.fixedLeftWidth);
    $datatable.find('.datatable-span.fixed-right').css('width',
        options.fixedRightWidth);
  ...
  $.fn.datatable = function (option, newData) {
    return this.each(function () {
      ...
      if (typeof option == 'string') {
        if (option === 'load' && $.isPlainObject(newData) && (newData.keepSort
            === undefined || newData.keepSort
            === null)) {
          newData.keepSort = true;
        }
        data[option](newData);
      }
    });
