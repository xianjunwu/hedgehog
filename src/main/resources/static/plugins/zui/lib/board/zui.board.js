...
(function ($) {
  ...
  Boards.prototype.getOptions = function (options) {
    options = $.extend({lang: $.zui.clientLang()}, Boards.DEFAULTS,
        this.$.data(), options);
    return options;
  };
  ...
  Boards.prototype.getLang = function () {
    ...
    this.lang = options.langs[options.lang]
        || options.langs[Boards.DEFAULTS.lang];
  };
  ...
  Boards.prototype.init = function () {
    ...
    var lang = this.lang;
    this.$.find(
        '.board-item:not(".disable-drop"), .board:not(".disable-drop")').each(
        function () {
          var $this = $(this);
          if ($this.attr('id')) {
          ...
          if ($this.hasClass('board')) {
            $this.find('.board-list').append(
                '<div class="board-item board-item-empty"><i class="icon-plus"></i> {append2end}</div>'.format(
                    lang))
            .append('<div class="board-item board-item-shadow"></div>'.format(
                lang));
          }
        });
  ...
  Boards.prototype.bind = function (items) {
    ...
    $boards.droppable($.extend({
      ...
      start: function (e) {
        $boards.addClass('dragging').find('.board-item-shadow').height(
      ...
      drag: function (e) {
        ...
        if (e.isIn) {
          ...
          $boards.addClass('drop-in').find('.board.drop-in').not(
              board).removeClass('drop-in');

          shadow.insertBefore(target);

          board.toggleClass('drop-in-empty',
              target.hasClass('board-item-empty'));
        }
      },
      drop: function (e) {
        if (e.isNew) {
      ...
      finish: function () {
        $boards.removeClass('dragging').removeClass('drop-in').find(
            '.board.drop-in').removeClass('drop-in');
      }
    }, setting.droppable));
  };
  ...
  $.fn.boards = function (option) {
    return this.each(function () {
      ...
      if (!data) {
        $this.data('zui.boards', (data = new Boards(this, options)));
      }
