...
(function () {
      ...
      if (option.nodeName.toUpperCase() === "OPTION") {
        if (option.text !== "") {
          ...
          this.parsed.push({
            ...
            search_keys: ($.trim(option.getAttribute('data-keys') || '')
  ...
  AbstractChosen = (function () {
    function AbstractChosen(form_field, options) {
      ...
      this.lang = LANGUAGE[this.options.lang || ($.zui.clientLang
          ? $.zui.clientLang() : 'zh_cn')];
    ...
    AbstractChosen.prototype.set_default_values = function () {
      ...
      && (this.form_field.options[0] != null) && this.form_field.options[0].text
      ...
      if (typeof max_drop_width === 'string' && max_drop_width.indexOf('px')
          === (max_drop_width.length - 2)) {
        max_drop_width = parseInt(
            max_drop_width.substring(0, max_drop_width.length - 2));
      }
      this.max_drop_width = max_drop_width;
      return this.display_disabled_options = this.options.display_disabled_options
    ...
    AbstractChosen.prototype.set_default_text = function () {
      ...
      return this.results_none_found = this.form_field.getAttribute(
          "data-no_results_text") || this.options.no_results_text
          || this.lang.no_results_text || AbstractChosen.default_no_result_text;
    };
    ...
    AbstractChosen.prototype.winnow_results = function (canMiddleHighlight) {
      var escapedSearchText, option, regex, regexAnchor, results, results_group,
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        if (this.include_option_in_results(option)) {
          ...
          if (!(option.group && !this.group_search)) {
            ...
            option.search_keys_match = this.search_string_match(
                option.search_keys, regex);
            option.search_text_match = this.search_string_match(
                option.search_text, regex);
            option.search_match = option.search_text_match
                || option.search_keys_match;
            ...
            if (option.search_match) {
              if (option.search_text_match && option.search_text.length) {
                ...
                text = option.search_text.substr(0,
                    startpos + searchText.length) + '</em>'
                    + option.search_text.substr(startpos + searchText.length);
                option.search_text = text.substr(0, startpos) + '<em>'
                    + text.substr(startpos);
              } else if (option.search_keys_match
                  && option.search_keys.length) {
                startpos = option.search_keys.search(zregex);
                text = option.search_keys.substr(0,
                    startpos + searchText.length) + '</em>'
                    + option.search_keys.substr(startpos + searchText.length);
                option.search_text += '&nbsp; <small style="opacity: 0.7">'
    ...
    AbstractChosen.prototype.search_string_match = function (search_string,
      ...
      if (regex.test(search_string)) {
        return true;
      } else if (this.enable_split_word_search && (search_string.indexOf(" ")
          >= 0 || search_string.indexOf("[") === 0)) {
    ...
    AbstractChosen.prototype.container_width = function () {
      ...
      } else if (this.form_field && this.form_field.classList
          && this.form_field.classList.contains('form-control')) {
        return '100%';
      } else {
  ...
  Chosen = (function (_super) {
    ...
    Chosen.prototype.set_up_html = function () {
      ...
      if (this.is_multiple) {
        ...
        if (this.compact_search) {
          this.container.addClass('chosen-compact').find(
              '.chosen-search').appendTo(this.container.find('.chosen-single'));
        } else {
          this.container.find('.chosen-search').prependTo(
              this.container.find('.chosen-drop'));
        }
      ...
      if (this.is_multiple) {
        ...
        this.search_container = this.container.find(
            'div.chosen-search').first();
        this.selected_item = this.container.find('.chosen-single').first();
      }
      if (this.options.drop_width) {
        this.dropdown.css('width', this.options.drop_width).addClass(
    ...
    Chosen.prototype.container_mousedown = function (evt) {
      if (!this.is_disabled) {
        ...
        if (!((evt != null) && ($(evt.target)).hasClass(
            "search-choice-close"))) {
          if (!this.active_field) {
            if (this.is_multiple) {
            ...
            this.results_show();
              ...
              === this.selected_item[0]) || $(evt.target).parents(
              "a.chosen-single").length)) {
            evt.preventDefault();
            this.results_toggle();
          }
          return this.activate_field();
    ...
    Chosen.prototype.result_do_highlight = function (el, canMiddleHighlight) {
      ...
      if (el.length) {
        ...
        if (this.middle_highlight && (canMiddleHighlight
            || this.middle_highlight === 'always')) {
          scrollTop = Math.min(high_top - resultHeight,
              Math.max(0, high_top - (maxHeight - resultHeight) / 2));
        } else if (high_bottom >= visible_bottom) {
          scrollTop = (high_bottom - maxHeight) > 0 ? high_bottom - maxHeight
              : 0;
    ...
    Chosen.prototype.results_show = function () {
      var that = this;
      if (that.is_multiple && that.max_selected_options
          <= that.choices_count()) {
      ...
      if (dropDirection === 'auto') {
        if (!that.drop_directionFixed) {
          ...
          if (offset.top + $drop.outerHeight() + 30 > $(window).height() + $(
              window).scrollTop()) {
            dropDirection = 'up';
          }
      ...
      that.container.toggleClass('chosen-up', dropDirection === 'up').addClass(
          "chosen-with-drop");
    ...
    Chosen.prototype.result_select = function (evt) {
      ...
      if (this.result_highlight) {
        ...
        item = this.results_data[high[0].getAttribute(
    ...
    Chosen.prototype.keydown_backstroke = function () {
      ...
      if (this.pending_backstroke) {
        ...
        if (next_available_destroy.length && !next_available_destroy.hasClass(
