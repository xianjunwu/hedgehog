...
(function ($, window) {
  ...
  Calendar.DEFAULTS = {
    langs: {
      ...
      en: {
        ...
        monthNames: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'June', 'July', 'Aug',
          'Sep', 'Oct', 'Nov', 'Dec'],
        today: 'Today',
        year: '{0}',
        month: '{0}',
  ...
  Calendar.prototype.getOptions = function (options) {
    this.options = $.extend(true, {}, Calendar.DEFAULTS, this.$.data(), options,
        true);
  };

  Calendar.prototype.getLang = function () {
    this.lang = this.options.langs[(this.options.lang || ($.zui
    && $.zui.clientLang ? $.zui.clientLang() : 'zh_cn')).replace('-', '_')];
  };
  ...
  Calendar.prototype.display = function (view, date) {
    ...
    var beforeDisplayResult = that.$.callComEvent(that, 'beforeDisplay',
        [eventPramas, doDisplay]);
  ...
  Calendar.prototype.displayMonth = function (date) {
    ...
    if (!$view || !$view.length) {
      ...
      for (i = 0; i < 7; i++) {
        $('<th>' + lang.weekNames[i] + '</th>').toggleClass('weekend-head',
            i >= 5).appendTo($weekHead);
      }

      for (i = 0; i < 6; i++) {
        $tr = $('<tr class="week-days"></tr>');
        for (var j = 0; j < 7; j++) {
          $('<td class="cell-day"><div class="day"><div class="heading"><span class="month"></span> <span class="number"></span></div><div class="content"><div class="events"></div></div></div></td>').toggleClass(
              'weekend-day', j >= 5).appendTo($tr);
        }
        $monthDays.append($tr);
      }
    ...
    $weeks.each(function (weekIdx) {
      ...
      $week.find('.day').each(function (dayIndex) {
        ...
        .text(((month === 0 && day === 1) ? (lang.year.format(year) + ' ') : '')
            + lang.monthNames[month]);
        $cell.toggleClass('current-month', month === thisMonth);
        $cell.toggleClass('current',
        ...
        if (dayEvents) {
          ...
          for (i = 0; i <= dayEvents.maxPos; ++i) {
            ...
            if (options.eventCreator) {
              ...
              $event = $(
                  '<div data-id="' + e.id + '" class="event" title="' + e.desc
                  + '"><span class="time">' + e.start.format('hh:mm')
                  + '</span> <span class="title">' + e.title + '</span></div>');
              $event.find('.time').toggle(!e.allDay);
              $event.data('event', e);
              $event.attr('data-days', e.days);
            }
            $event.toggleClass('event-all-day', !!e.allDay).data('event',
                e).attr('data-days', e.days);
    ...
    if (options.withHeader) {
      that.$caption.text(lang.yearMonth.format(thisYear, thisMonth + 1,
          lang.monthNames[thisMonth]));
      that.$todayBtn.toggleClass('disabled',
          thisMonth === todayMonth && thisYear === todayYear);
    }
    ...
    if (options.dragThenDrop) {
      ...
      if (!$view.data('zui.droppable')) {
        ...
        }, ($.isPlainObject(options.dragThenDrop)) ? options.dragThenDrop
            : null));
      }
    }
  };
