...
(function ($, window, document, Math, undefined) {
  ...
  var Treemap = function (element, options) {
    ...
    if (!$nodes.length) {
      $nodes = $('<div class="treemap-nodes" unselectable="on"/>').appendTo(
          $element);
    }
    ...
    if (options.foldable) {
      $nodes.on('click', options.clickNodeToFold ? '.treemap-node-wrapper'
          : '.treemap-node-fold-icon', function () {
  ...
  Treemap.prototype.toggle = function ($node, toggle, ignoreAnimation) {
    ...
    if ($node) {
      ...
      $node.toggleClass('collapsed', !toggle).find(
  ...
  Treemap.prototype.createNodes = function (nodes, parent, callback) {
    ...
    if (options.sort) {
      nodes.sort(
          $.isFunction(options.sort) ? options.sort : function (nodeX, nodeY) {
    ...
    $.each(nodes, function (idx, node) {
      ...
      var $node = isCustomNodeTemplate ? options.nodeTemplate(node, that) : $(
  ...
  Treemap.prototype.drawLines = function (nodes, parent) {
    var that = this,
        ...
        rowSpace = options.rowSpace;
    var cableStyle = {};
    if (options.cableWidth) {
      cableStyle.borderWidth = options.cableWidth;
    }
    if (options.cableStyle) {
      cableStyle.borderStyle = options.cableStyle;
    }
    if (options.cableColor) {
      cableStyle.borderColor = options.cableColor;
    }
    ...
    $.each(nodes || that.data, function (idx, node) {
      ...
      if (children && children.length) {
        ...
        if (!$bottomLine.length) {
          $bottomLine = $('<div class="treemap-line-bottom"/>').appendTo(
              $wrapper);
          if (options.foldable) {
            $bottomLine.append(
                '<i class="treemap-node-fold-icon icon" style="transform: translate(-'
                + Math.floor(nodeCableStyle.borderWidth / 2) + 'px, '
                + rowSpaceHalf + 'px)"/>');
          }
        }
        ...
        if (children.length > 1) {
          ...
          if (!$centerLine.length) {
            $centerLine = $('<div class="treemap-line"/>').insertAfter(
                $wrapper);
          }
          var lineLeft = Math.round(
              firstChild.$wrapper.offset().left - nodesOffsetLeft
              + firstChild.$wrapper.outerWidth() / 2);
          $centerLine.css($.extend({
            marginTop: rowSpaceHalf,
            left: lineLeft,
            width: lastChild.$wrapper.offset().left - nodesOffsetLeft - lineLeft
                + lastChild.$wrapper.outerWidth() / 2
          }, cableStyle));
        }
      }
    });
  ...
  $.fn.treemap = function (option, param1, parma2) {
    return this.each(function () {
      ...
      if (!data) {
        $this.data(NAME, (data = new Treemap(this, options)));
      }

      if (typeof option == 'string') {
        data[option](param1, parma2);
      }
    });
  };
