...
(function ($) {
  ...
  var isPointInner = function (x, y, a) {
    return x >= a.left && x <= (a.left + a.width) && y >= a.top && y <= (a.top
        + a.height);
  };
  ...
  var isIntersectArea = function (a, b) {
    ...
    return isPointInner(x1, y1, a) && isPointInner(x2, y2, a) && isPointInner(
        x1, y1, b) && isPointInner(x2, y2, b);
  };
  ...
  Selectable.DEFAULTS = {
    ...
    selectClass: 'active',
    rangeStyle: {
      border: '1px solid ' + ($.zui.colorset ? $.zui.colorset.primary
          : '#3280fc'),
      backgroundColor: $.zui.colorset ? (new $.zui.Color(
          $.zui.colorset.primary).fade(20).toCssStr())
          : 'rgba(50, 128, 252, 0.2)'
    },
  ...
  Selectable.prototype.syncSelectionsFromClass = function () {
    ...
    that.$children.each(function () {
      var $item = $(this);
      that.selections[$item.data('id')] = $item.hasClass(
  ...
  Selectable.prototype._init = function () {
    ...
    var checkRange = function () {
      ...
      that.$children.each(function () {
        ...
        var isIntersect = rangeFunc ? rangeFunc.call(this, range, offset)
            : isIntersectArea(range, offset);
        if (checkFunc) {
    ...
    var mousemove = function (e) {
      ...
      if (isIgnoreMove && range.width < ignoreVal && range.height
          < ignoreVal) {
        return;
      }
      if (!$range) {
        ...
        if (!$range.length) {
          $range = $(
              '<div class="selectable-range" data-id="' + that.id + '"></div>')
          .css($.extend({
            zIndex: 1060,
            position: 'absolute',
            top: startX,
    ...
    var mouseup = function (e) {
      ...
      that.callEvent('finish',
          {selections: that.selections, selected: that.getSelectedArray()});
      e.preventDefault();
    };
    ...
    var mousedown = function (e) {
      ...
      if (that.altKey || e.which === 3 || that.callEvent('start', e)
          === false) {
        return;
      }
      ...
      $(document).on('mousemove' + eventNamespace, mousemove).on(
          'mouseup' + eventNamespace, mouseup);
    ...
    var $container = options.container && options.container !== 'default' ? $(
  ...
  $.fn.selectable = function (option) {
    return this.each(function () {
      ...
      if (!data) {
        $this.data(name, (data = new Selectable(this, options)));
      }
