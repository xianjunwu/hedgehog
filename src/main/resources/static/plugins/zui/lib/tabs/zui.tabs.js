...
(function ($) {
  ...
  var Tab = function (tab) {
    ...
    if (!that.type) {
      if (that.iframe) {
        ...
        that.type = 'ajax';
        that.url = that.url || ($.isPlainObject(that.ajax) ? that.ajax.url
            : that.ajax);
      } else if (that.url) {
  ...
  Tabs.prototype.createMenuItems = function (tab) {
    ...
    return [{
      ...
      disabled: !$('#tab-nav-item-' + tab.id).next(
          '.tab-nav-item:not(.hidden)').length,
      onClick: function () {
  ...
  Tabs.prototype.renderTab = function (tab, beforeTabId) {
    ...
    if (!$tabNav.length) {
      var $a = $(
          '<a class="tab-nav-link"><i class="icon"></i><span class="title"></span><i class="close tab-nav-close" title="'
          + that.lang.close + '">&times;</i></a>').attr({
        href: '#tabs-item-' + tab.id,
        'data-id': tab.id
      });
      $tabNav = $(
          '<li class="tab-nav-item" data-id="' + tab.id + '" id="tab-nav-item-'
          + tab.id + '" />').append($a).appendTo(that.$nav);
      if (beforeTabId) {
    ...
    var $a = $tabNav.find('a').attr('title', tab.desc).toggleClass(
        'not-closable', !!tab.forbidClose);
    $a.find('.icon').attr('class',
        'icon ' + (tab.icon || that.options.defaultTabIcon));
  ...
  Tabs.prototype.open = function (tab, forceReload) {
    ...
    if (!$tabPane.length) {
      $tabPane = $('<div class="tab-pane" id="tab-' + tab.id + '" />').appendTo(
          that.$tabs);
    }
  ...
  Tabs.prototype.reload = function (tab) {
    ...
    var $tabNav = $('#tab-nav-item-' + tab.id).addClass('loading').removeClass(
        'has-error');
    var $tabPane = $('#tab-' + tab.id).addClass('loading').removeClass(
        'has-error');
    ...
    if (tab.type === 'ajax') {
      var ajaxOption = {
        ...
        error: function (jqXHR, textStatus, errorThrown) {
          afterRefresh(false,
              that.lang.errorCannotFetchFromRemote.format(tab.url));
        },
      ...
      try {
        ...
        var $iframe = $(
            '<iframe id="' + iframeName + '" name="' + iframeName + '" src="'
            + (tab.url)
  ...
  $.fn.tabs = function (option) {
    return this.each(function () {
      ...
      if (!data) {
        $this.data(NAME, (data = new Tabs(this, options)));
      }
