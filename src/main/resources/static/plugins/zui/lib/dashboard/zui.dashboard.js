...
(function ($, Math, undefined) {
  ...
  var dashboardMessager = $.zui.Messager ? new $.zui.Messager(
      {placement: 'top', time: 1500, close: 0, scale: false, fade: false}) : 0;

  var Dashboard = function (element, options) {
    this.$ = $(element);
    this.options = this.getOptions(options);
    this.draggable = this.$.hasClass('dashboard-draggable')
  ...
  Dashboard.prototype.handleRemoveEvent = function () {
    ...
    this.$.on('click', '.remove-panel', function () {
      ...
      var name = panel.data('name') || panel.find(
          '.panel-heading').text().replace('\n', '').replace(/(^\s*)|(\s*$)/g,
          '');
    ...
    this.$.on('mousedown', '.panel-heading, .panel-drag-handler',
        function (event) {
          ...
          if (!$dColShadow.length) {
            $dColShadow = $(
                '<div class="dragging-col-holder"><div class="panel"></div></div>').removeClass(
                'dragging-col').appendTo(row);
          }
          ...
          $dColShadow.insertBefore(pCol).find('.panel').replaceWith(
              panel.clone().addClass('panel-dragging panel-dragging-holder'));
          ...
          function mouseMove(event) {
            ...
            row.children(':not(.dragging-col)').each(function () {
              ...
              if (options.sensitive) {
                pX -= dPos.left;
                pY -= dPos.top;
                thisArea = getIntersectArea(sX1, sY1, sX2, sY2, pX, pY, pX + pW,
                    pY + pH);
                if (thisArea > 100 && thisArea > area && thisArea > Math.min(
                    getRectArea(sX1, sY1, sX2, sY2),
                    getRectArea(pX, pY, pX + pW, pY + pH)) / 3) {
                  area = thisArea;
                  dropCol = col;
                }
              } else {
  ...
  Dashboard.prototype.handlePanelPadding = function () {
    this.$.find(
        '.panel-body > table, .panel-body > .list-group').parent().addClass(
        'no-padding');
  };
  ...
  Dashboard.prototype.updatePanelHeight = function () {
    ...
    if (that.id && $.zui.store) {
      sizeConfig = $.zui.store.pageGet(
          'zui.dashboard.' + that.id + '.sizeConfig', sizeConfig);
    }
        ...
        $.each(rows, function (rowId) {
          ...
          $.each(row, function (colId) {
            ...
            if (newHeight) {
              ...
              var panelHeight = $panel.data('height') || sizeConfig[$panel.data(
                  'id')];
              if (panelHeight) {
                bestHeight = Math.max(bestHeight, panelHeight);
              }
            }
          });
          if (!bestHeight) {
    ...
    if (that.id && $.zui.store) {
      $.zui.store.pageSet('zui.dashboard.' + that.id + '.sizeConfig',
          sizeConfig);
    }
  ...
  Dashboard.prototype.handleResizeEvent = function () {
    ...
    that.$.on('mousedown', '.resize-handle', function (e) {
      ...
      var mouseMove = function (event) {
        if (isVertical) {
          $panel.css('height',
              Math.max(minHeight, startHeight + (event.pageY - startY)));
        } else {
          var x = event.pageX;
          var grid = Math.max(1, Math.min(12,
              Math.round(12 * (startWidth + (x - startX)) / rowWidth)));
          if (lastGrid != grid) {
            $col.attr('data-grid', grid).css('width', (100 * grid / 12) + '%');
            if (messagerAvaliable) {
              dashboardMessager[dashboardMessager.isShow
                  ? 'update' : 'show'](
                  Math.round(100 * grid / 12) + '% (' + grid + '/12)');
            }
            lastGrid = grid;
          }
        }
      ...
      var mouseUp = function (event) {
        ...
        if (isVertical) {
          var newHeight = Math.max(minHeight,
              startHeight + (event.pageY - startY));
          ...
          if (oldGrid != lastGrid) {
            if ($.isFunction(onResize)) {
              ...
              if (result === false) {
                ...
                if (messagerAvaliable) {
                  dashboardMessager.show(
                      Math.round(100 * lastGrid / 12) + '% (' + lastGrid
                      + '/12)');
                }
              }
            }
        ...
        that.updatePanelHeight();
        $('body').off('mousemove.resize', mouseMove).off('mouseup.resize',
            mouseUp);
        event.preventDefault();
        event.stopPropagation();
      };
  ...
  Dashboard.prototype.refresh = function ($panel, onlyRefreshBody) {
    if (onlyRefreshBody
        === undefined) {
      onlyRefreshBody = this.options.onlyRefreshBody;
    }
    ...
    $panel.addClass('panel-loading').find(
        '.panel-heading .icon-refresh,.panel-heading .icon-repeat').addClass(
        'icon-spin');
    $.ajax({
      url: url,
      dataType: 'html'
      ...
      $panel.find(
          '.panel-heading .icon-refresh,.panel-heading .icon-repeat').removeClass(
          'icon-spin');
    });
  };
  ...
  function getIntersectArea(ax1, ay1, ax2, ay2, bx1, by1, bx2, by2) {
    var x1 = Math.max(ax1, bx1),
        ...
        x2 = Math.min(ax2, bx2),
        y2 = Math.min(ay2, by2);
    if (isPointInner(x1, y1, ax1, ay1, ax2, ay2) && isPointInner(x2, y2, ax1,
        ay1, ax2, ay2) && isPointInner(x1, y1, bx1, by1, bx2, by2)
        && isPointInner(x2, y2, bx1, by1, bx2, by2)) {
  ...
  Dashboard.prototype.init = function () {
    ...
    if (options.data) {
      ...
      $.each(options.data, function (idx, config) {
        ...
        if (config.colAttrs) {
          $col.attr(config.colAttrs);
        }
        var $panel = $(
            '<div class="panel" data-id="' + (config.id || $.zui.uuid())
            + '"/>');
        if (config.panelAttrs) {
          $panel.attr(config.panelAttrs);
        }
        if (config.height !== undefined) {
          $panel.data('height', config.height);
        }
  ...
  $.fn.dashboard = function (option) {
    return this.each(function () {
      ...
      if (!data) {
        $this.data('zui.dashboard',
            (data = new Dashboard(this, options)));
      }
