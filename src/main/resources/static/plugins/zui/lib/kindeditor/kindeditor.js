...
(function (window, undefined) {
  ...
  if (_IE) {
    window[window.attachEvent ? 'attachEvent' : 'addEventListener']('onunload',
        function () {
  ...
  function _formatHtml(html, htmlTags, urlType, wellFormatted, indentChar,
    ...
    html = html.replace(re, function ($0, $1, $2, $3, $4, $5, $6) {
      ...
      if (wellFormatted && (tagName == 'br' || tagName === 'hr') && (!simpleWrap
          || prevTagIsBlockEnd === true)) {
        endNewline = '\n';
      }
      if (_BLOCK_TAG_MAP[tagName] && !_PRE_TAG_MAP[tagName]) {
        if (wellFormatted) {
          var isEndTag = !!(startSlash && tagStack.length > 0
          ...
          if (!simpleWrap || (prevTagIsBlockEnd === false && !isEndTag)
              || (prevTagIsBlockEnd === true)) {
            for (var i = 0,
                len = startSlash ? tagStack.length : tagStack.length - 1;
                i < len; i++) {
  ...
  _extend(KUploadButton, {
    init: function (options) {
      ...
      var html = [
        ...
        + '" tabindex="-1" />', (options.form ? '</div>' : '</form>'),
        '</div>'
      ].join('');
  ...
  function _create(expr, options) {
    ...
    if (_undef(options.loadStyleMode, K.options.loadStyleMode)) {
      _loadStyle(_undef(options.styleCssPath,
          options.basePath + '/kindeditor.min.css'));
  ...
  self.plugin.multiImageDialog = function (options) {
    ...
    var swfupload = K.swfupload({
      ...
      buttonImageUrl: imgPath + (self.langType == 'zh-CN'
          ? 'select-files-zh-CN.png' : 'select-files-en.png'),
...
$.each(['afterBlur', 'afterFocus', 'afterChange', 'afterTab'],
...
KindEditor.EditorClass.prototype.setPlaceholder = function (placeholder,
    asHtml) {
  var self = this;
  var options = self.options;
  var edit = self.edit;
  ...
  self.afterCreate(function () {
    ...
    if (self.setPlaceholder) {
      ...
      if (placeholder) {
        ...
        } else if (oldPlaceholder.indexOf(placeholder)
            < 0) {
    ...
    var pasteBegin = function () {
      ...
      var imageLoadingEle = '<div class="image-loading-ele small" style="padding: 5px; background: #FFF3E0; width: 300px; border-radius: 2px; border: 1px solid #FF9800; color: #ff5d5d; margin: 10px 0;"><i class="icon icon-spin icon-spinner-indicator muted"></i> '
          + lang.uploadingHint + '</div>';
    ...
    $(doc.body).on('paste.ke' + uuid, function (ev) {
      if (K.WEBKIT) {
        ...
        var clipboardItems = original.clipboardData
            && original.clipboardData.items;
...
KindEditor.plugin('table', function (K) {
  ...
  var $elements = [];
  var lang = $.extend({}, self.lang('table.'),
      allLangs[($.clientLang || $.zui.clientLang)()]);
  ...
  function _setColor(box, color) {
    ...
    if (color) {
      if ($ && $.zui && $.zui.Color) {
        ...
        box.css('color',
            (color === '#FFF' || color === '#FFFFFF') ? '#000' : '#FFF');
      }
    }
        ...
        if (setting.header) {
          if (!$thead.length) {
            ...
            for (var i = 0; i < colsCount; ++i) {
              theadHtml.push(
                  '<th style="background-color: #f1f1f1; border: 1px solid '
                  + (setting.borderColor || defaultTableBorderColor) + '">'
                  + (K.IE ? '&nbsp;' : '<br />') + '</th>');
            }
    ...
    if (setting.stripedRows !== undefined) {
      ...
      $rows.each(function (index) {
        $(this).css('background-color',
            (setting.stripedRows && (index % 2 === 0)) ? '#f9f9f9' : 'none');
      });
  ...
  function insertTable(row, col, headerRow, headerCol) {
    ...
    var $table = $('<table id="' + tableID
        + '" class="table table-kindeditor" style="width: 100%"></table>');
    var $body = $('<tbody></tbody>');
    for (var r = 0; r < row; r++) {
      var $row = $('<tr></tr>');
      for (var c = 0; c < col; c++) {
        var $cell = $(
            '<td style="border: 1px solid ' + defaultTableBorderColor + '">'
            + (K.IE ? '&nbsp;' : '<br />') + '</td>');
        $row.append($cell);
      }
      $body.append($row);
    ...
    $table.attr('id', null);
    self.cmd.range.selectNodeContents($table.find('th,td').first()[0]).collapse(
        true);
    self.cmd.select();
    self.addBookmark();
    return $table;
  ...
  function modifyTable(table) {
    ...
    var dialogHtml = [
      ...
      '<input class="form-control ke-plugin-table-input-color" readonly style="background: '
      + defaultTableBorderColor + '; color: #333; font-size: 12px" value="'
      + defaultTableBorderColor + '" name="borderColor" />',
      '</div>',
      '</div>',
      '</div>',
    ...
    $dialog.on('change.kTable', 'input[name]', function () {
      var $input = $(this);
      var updateSetting = {};
      updateSetting[$input.attr('name')] = $input.is('[type="checkbox"]')
    ...
    updateTable(self.tableSetting, $exampleTable, function (name, value) {
      switch (name) {
        ...
        case 'autoWidth':
          $dialog.find(
              '[name="autoWidth"][value="' + (value ? 'auto' : '') + '"]').prop(
              'checked', true);
          break;
      }
    });
  ...
  if (!self.plugin.table) {
    self.plugin.table = {
      ...
      cellprop: function () {
        var html = [
          ...
          //width, height
          '<div class="form-group">',
          '<label class="col-xs-2" style="margin: 0;">' + lang.size
          + '</label>',
          '<div class="col-xs-5">',
          ...
          //align
          '<div class="form-group">',
          '<label class="col-xs-2" style="margin: 0;">' + lang.align
          + '</label>',
          '<div class="col-xs-5">',
          '<div class="input-group">',
          ...
          //border
          '<div class="form-group">',
          '<label class="col-xs-2" style="margin: 0;">' + lang.border
          + '</label>',
          '<div class="col-xs-5">',
          '<div class="input-group">',
          ...
          //background color
          '<div class="form-group">',
          '<label class="col-xs-2" style="margin: 0;">' + lang.color
          + '</label>',
          '<div class="col-xs-5">',
          '<div class="input-group">',
        ...
        var bookmark = self.cmd.range.createBookmark();
        var div, widthBox, heightBox, widthTypeBox, widthTypeBox, textAlignBox,
            verticalAlignBox, colorBox, borderWidthBox;
        var dialog = self.createDialog({
          name: name,
          width: 500,
      ...
      colinsert: function (offset) {
        ...
        for (var i = 0, len = table.rows.length; i < len; i++) {
          ...
          newCell.outerHTML = '<' + (isThead ? 'th' : 'td') + (newCell.rowSpan
              > 1 ? ' rowspan="' + newCell.rowSpan + '"' : '') + (newCell.colSpan
              > 1 ? ' colspan="' + newCell.colSpan + '"' : '') + ' style="'
              + (isThead ? 'background-color: #f1f1f1;' : '')
              + 'border: 1px solid ' + ((self.tableSetting
                  && self.tableSetting.borderColor) || defaultTableBorderColor)
              + '">' + (K.IE ? '&nbsp;' : '<br />') + '</' + (isThead ? 'th'
                  : 'td') + '>';
          // 调整下一行的单元格index
        ...
        $table.children('thead,tbody,tfoot').children('tr').each(function () {
          $(this).children('td,th').each(function () {
            ...
            if (pos.left === left && pos.top === top) {
              $firstCell = $cell;
            } else if (pos.right >= left && pos.left <= right && pos.bottom
                >= top && pos.top <= bottom) {
      ...
      rowsplit: function () {
        ...
        for (var i = 1, len = cell.rowSpan; i < len; i++) {
          ...
          newCell.outerHTML = '<' + (isThead ? 'th' : 'td') + (newCell.rowSpan
              > 1 ? ' rowspan="' + newCell.rowSpan + '"' : '') + (colSpan > 1
              ? ' colspan="' + colSpan + '"' : '') + ' style="' + (isThead
              ? 'background-color: #f1f1f1;' : '') + 'border: 1px solid '
              + ((self.tableSetting && self.tableSetting.borderColor)
                  || defaultTableBorderColor) + '">' + (K.IE ? '&nbsp;'
                  : '<br />') + '</' + (isThead ? 'th' : 'td') + '>';
          if (cell.colSpan > 1) {
      ...
      colsplit: function () {
        ...
        for (var i = 1, len = cell.colSpan; i < len; i++) {
          ...
          newCell.outerHTML = '<' + (isThead ? 'th' : 'td') + (rowSpan > 1
              ? ' rowspan="' + rowSpan + '"' : '') + (colSpan > 1 ? ' colspan="'
              + colSpan + '"' : '') + ' style="' + (isThead
              ? 'background-color: #f1f1f1;' : '') + 'border: 1px solid '
              + ((self.tableSetting && self.tableSetting.borderColor)
                  || defaultTableBorderColor) + '">' + (K.IE ? '&nbsp;'
                  : '<br />') + '</' + (isThead ? 'th' : 'td') + '>';
    ...
    K.each(
        ('prop,cellprop,colinsertleft,colinsertright,rowinsertabove,rowinsertbelow,mergeCells,rowmerge,colmerge,rowsplit,colsplit,coldelete,rowdelete,delete').split(
            ','), function (i, val) {
          var cond;
          ...
          } else if (K.inArray(val,
              ['colinsertleft', 'colinsertright', 'rowinsertabove',
                'rowinsertbelow']) > -1) {
  ...
  self.clickToolbar(name, function () {
    ...
    var $header = $(
        '<div class="ke-plugin-table-header" style="padding: 0 5px;">'
        + lang.xRxC.format(0, 0) + '</div>');
    $wrapperDiv.append($header);
    var $grid = $(
        '<div class="ke-plugin-table-grid clearfix" style="width: 230px; padding: 5px 5px 0 5px"></div>');
    ...
    for (var r = 1; r < 11; r++) {
      for (var c = 1; c < 11; c++) {
        $grid.append(
            '<div class="ke-plugin-table-grid-cell" style="float: left; width: 20px; height: 20px;  margin: 1px; border: 1px solid #ddd; background: #f1f1f1;" data-row="'
            + r + '" data-col="' + c + '"></div>');
      }
    }
    $elements.push($grid);
      ...
      var selectNextCell = function ($currentCell) {
        if ($currentCell.length) {
          ...
          if (!$nextCell.is('td,th')) {
            $nextCell = $currentCell.parent().next('tr').children(
                'th,td').first();
          }
          if (!$nextCell.is('td,th')) {
            $nextCell = $currentCell.closest(
                'tbody,tfoot,thead').next().children('tr').first().children(
                'th,td').first();
          }
          if ($nextCell.length) {
  ...
  var selectCellsRange = function ($table, startPos, endPos) {
    ...
    var bottom = endPos ? Math.max(startPos.bottom, endPos.bottom)
        : startPos.bottom;
    var right = endPos ? Math.max(startPos.right, endPos.right)
        : startPos.right;
    ...
    var $rows = $table.children('thead,tbody,tfoot').children('tr').each(
        function () {
          $(this).children('td,th').each(function () {
            var $cell = $(this);
            var pos = $cell.cellPos();
            if (pos.right >= left && pos.left <= right && pos.bottom >= top
                && pos.top <= bottom) {
    ...
    while (hasNewCellSelect) {
      ...
      $rows.each(function () {
        $(this).children('td,th').each(function () {
          var $cell = $(this);
          if ($cell.hasClass('ke-select-cell')) {
            return;
          }
          var pos = $cell.cellPos();
          if (pos.right >= left && pos.left <= right && pos.bottom >= top
  ...
  self.afterCreate(function () {
    ...
    $(self.edit.doc.body).on('mousedown.ke' + self.uuid, function (e) {
      ...
      var $cell = $(e.target).closest('td,th');
      if (!$cell.length) {
        return isMouseDown ? e.preventDefault() : null;
      }
      var $table = $cell.closest('table');
      if (!$table.length) {
        return isMouseDown ? e.preventDefault() : null;
      ...
      if (isMouseDown) {
        if ($table[0] !== $mouseDownTable[0]) {
          return e.preventDefault();
        }
        $(self.edit.doc).find('table').find('.ke-select-cell').removeClass(
            'ke-select-cell');
      ...
      if ($cell.length) {
        if (mouseMoveCellPos && mouseMoveCellPos.selectRow !== undefined) {
          ...
          e.stopPropagation();
        } else if (mouseMoveCellPos && mouseMoveCellPos.selectCol
            !== undefined) {
      ...
      $(self.edit.doc).find('table').removeClass(
          'ke-select-row ke-select-col').find('.ke-select-cell').removeClass(
          'ke-select-cell');
    });
    ...
    var eachSelectCells = function (eachCallback, beforeCallback,
        afterCallback) {
      var range = self.cmd.range;
      if (range && range.endContainer) {
        ...
        var $selectCells = $table.children('thead,tbody,tfoot').children(
            'tr').children('.ke-select-cell');
        if ($selectCells.length) {
          if (beforeCallback) {
            beforeCallback($cell, $table);
          }
          $selectCells.each(eachCallback);
          if (afterCallback) {
            afterCallback($cell, $table);
          }
