...
// span level renderer
  ...
  function unescape(html) {
    ...
    return html.replace(/&(#(?:\d+)|(?:#x[0-9A-Fa-f]+)|(?:\w+));?/g,
        function (_, n) {
          n = n.toLowerCase();
          if (n === 'colon') {
            return ':';
...
(function ($) {
  ...
  MarkDoc.prototype.getContent = function (callback, options) {
    ...
    if (content) {
      ...
      var ajaxOptions = $.isPlainObject(options.remote) ? options.remote
          : {url: options.remote};
      ...
      var $source = $.isFunction(options.source) ? options.source(that) : $(
          options.source);
  ...
  $.fn.markdoc = function (option, param) {
    return this.each(function () {
      ...
      if (!data) {
        $this.data(NAME, (data = new MarkDoc(this, options)));
      }
