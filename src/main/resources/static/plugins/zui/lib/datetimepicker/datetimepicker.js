...
!function ($) {
  ...
  var Datetimepicker = function (element, options) {
    ...
    this.language = (options.language || this.element.data('date-language')
        || ($.zui && $.zui.clientLang ? $.zui.clientLang().replace('_', '-')
            : "zh-cn")).toLowerCase();
    ...
    this.component = this.element.is('.date') ? this.element.find(
        '.input-group-addon .icon-th, .input-group-addon .icon-time, .input-group-addon .icon-calendar').parent()
        : false;
    this.componentReset = this.element.is('.date') ? this.element.find(
        '.input-group-addon .icon-remove').parent() : false;
  ...
  Datetimepicker.prototype = {
    ...
    place: function () {
      ...
      if (this.component) {
        ...
        if (this.pickerPosition === 'bottom-left' || this.pickerPosition
            === 'top-left' || this.pickerPosition === 'auto-left') {
      ...
      var realPickerPosition = isAutoPosition ? (((offset.top
          + this.picker.outerHeight() > $(window).height() + $(
              window).scrollTop()) ? 'top' : 'bottom')
          + (this.pickerPosition.lastIndexOf('-left') === 0 ? '-left'
              : '-right')) : this.pickerPosition;

      if (realPickerPosition === 'top-left' || realPickerPosition
          === 'top-right') {
      ...
      this.picker.css({
        ...
        left: left,
        zIndex: zIndex
      }).attr('class', 'datetimepicker dropdown-menu datetimepicker-dropdown-'
          + realPickerPosition);
      if (this.pickerClass) {
        this.picker.addClass(this.pickerClass);
      }
      if (this.pickerId) {
        this.picker.attr('id', this.pickerId);
      }
    },
    ...
    update: function () {
      ...
      if (arguments && arguments.length && (typeof arguments[0] === 'string'
        ...
        date = this.element.data('date') || (this.isInput ? this.element.val()
            : this.element.find('input').val()) || this.initialDate;
      ...
      switch (this.viewMode) {
        case 0:
          if (this.startDate !== -Infinity && year
              ...
              <= this.startDate.getUTCMonth() && day
              <= this.startDate.getUTCDate() && hour
          ...
          if (this.endDate !== Infinity && year >= this.endDate.getUTCFullYear()
              && month >= this.endDate.getUTCMonth() && day
              >= this.endDate.getUTCDate() && hour
    ...
    return this.each(function () {
      ...
      if (!data) {
        $this.data('datetimepicker', (data = new Datetimepicker(this,
            $.extend({}, $.fn.datetimepicker.defaults, $this.data(),
                options))));
      }
