...
(function ($, undefined) {
  ...
  var loadDataSourceFromTable = function ($table) {
    ...
    $table.find('thead>tr:first>th').each(function (idx) {
      ...
      if ($th.attr('colspan') && $th.attr('colspan') !== '1') {
        throw new Erorr(
            'Table th element with colspan attribute is not support.');
      }
    });
    var data = [];
  ...
  var DataGrid = function (element, options) {
    ...
    options.valueOperator = $.extend({}, DEFAULT_VALUE_OPERATOR,
        options.valueOperator);
    options.rowDefaultHeight = options.rowDefaultHeight || 30;
    options.headerHeight = options.headerHeight || options.rowDefaultHeight
        || 30;
    ...
    // Initialize
    if ($element.is('table')) {
      options.dataSource = $.extend(loadDataSourceFromTable(that.$),
          options.dataSource);
      $element.hide();
      $element = $('<div class="datagrid" id="datagrid-' + that.uuid
          + '" />').insertAfter(that.$);
    }
    ...
    var createScrollbar = function (direction) {
      ...
      if (!$scrollbar.length) {
        $scrollbar = $(
            '<div class="datagrid-scrollbar datagrid-scrollbar-' + direction
            + '"><div class="bar"></div></div>').appendTo($container);
      }
      ...
      var handleMousePosition = function (e) {
        ...
        if (isClickBar) {
          ...
          offset = Math.max(0,
              Math.min(scroll.space, pos - Math.round(scroll.barSize / 2)));
        }
        if (direction === 'h') {
    ...
    $container.on('mousewheel', function (event) {
      ...
      var scrollLeft = layout.scrollLeft - Math.round(
          event.deltaX * mouseWheelFactor);
      var scrollTop = layout.scrollTop - Math.round(
          event.deltaY * mouseWheelFactor);
      scrollLeft = Math.max(0,
          Math.min(scrollLeft, layout.width - layout.containerWidth));
      scrollTop = Math.max(0,
          Math.min(scrollTop, layout.height - layout.containerHeight));
    ...
    that.configs = that.isFuncConfigs ? options.configs : $.extend({},
        DEFAULT_CONFIGS, options.configs);
    ...
    if (options.checkable) {
      if (options.selectable && $.fn.selectable) {
        that.selectable = $cells.selectable($.extend({
          ...
          // selectClass: false,
          trigger: options.checkByClickRow ? null
              : '.datagrid-row-cell .datagrid-has-checkbox',
          clickBehavior: 'multi',
        ...
        }, $.isPlainObject(options.selectable) ? options.selectable
            : null)).data('zui.selectable');
        $cells.on('click', '.datagrid-cell-head.datagrid-has-checkbox',
            function () {
              that.checkRow($(this).data('row'));
              that.selectable.syncSelectionsFromClass();
            });
      } else {
        $cells.on('click', options.checkByClickRow ? '.datagrid-row'
            : '.datagrid-has-checkbox', function (e) {
          var rowIndex = $(this).data('row');
          if (rowIndex || $(e.target).closest(
              '.datagrid-has-checkbox').length) {
  ...
  DataGrid.prototype.setPager = function (page, recTotal, recPerPage) {
    ...
    pager.totalPage = (pager.recTotal && pager.recPerPage) ? (Math.ceil(
        pager.recTotal / pager.recPerPage)) : 1;
    ...
    if (pager.page && pager.recTotal) {
      if (pager.page < pager.totalPage) {
        ...
        pager.pageRecCount = pager.recTotal - (pager.recPerPage * (pager.page
            - 1));
      }
    }
    ...
    if (oldPager.page !== pager.page || oldPager.recTotal !== pager.recTotal
        || oldPager.recPerPage !== pager.recPerPage) {
      that.scroll(0, 0);
      that.layout.cols = null;
    }
    return that;
  ...
  DataGrid.prototype.filterData = function (arr, filter) {
    ...
    if (result.length) {
      ...
      if (sortBy) {
        ...
        var sortFunc = (colConfig && colConfig.sortFunc)
            || that.options.sortFunc || DEFAULT_SORT_FUNC;
        result.sort(function (item1, item2) {
          var sortResult = sortFunc(item1[sortBy], item2[sortBy], item1, item2,
              sortBy, that);
  ...
  DataGrid.prototype.loadData = function (callback) {
    ...
    var dataId = [params.page, params.recPerPage, params.search, params.sortBy,
      params.order].join('&');
          ...
          $.ajax($.extend({
            ...
            success: function (responseData, textStatus, jqXHR) {
              if (dataSource.remoteConverter) {
                responseData = dataSource.remoteConverter(responseData,
                    textStatus, jqXHR, that);
              }
              ...
              if ($.isPlainObject(responseData) && responseData.data) {
                ...
                if (result === 'success' || result === 'ok' || result === 200) {
                  ...
                  onFinish(false, responseData.message || responseData.reason
                      || that.lang['errorCannotHandleRemoteData'],
                      responseData);
                }
              } else {
                onFinish(false, that.lang['errorCannotHandleRemoteData'],
                    responseData);
              }
            },
            error: function () {
              onFinish(false, that.lang['errorCannotGetDataFromRemote'].format(
                  dataSource.remote));
            },
          }, ajaxOptions));
        };
  ...
  DataGrid.prototype.showMessage = function (message, type, autoCloseTime) {
    ...
    if (!$messager.length) {
      $messager = $(
          '<div class="datagrid-messager" style="display: none"><div class="content"></div><button type="button" class="close">×</button></div>').appendTo(
          that.$container).on('click', '.close', function () {
        $messager.slideUp();
        if (that.msgerAutoCloseTimer) {
          clearTimeout(that.msgerAutoCloseTimer);
          that.msgerAutoCloseTimer = null;
    ...
    $messager.attr('class', 'datagrid-messager bg-' + type).find(
  ...
  DataGrid.prototype.renderLoading = function (loading) {
    ...
    if (loading) {
      if (!$loading.length) {
        $loading = $(
            '<div class="datagrid-loading" style="display: none"><div class="content"><i class="icon icon-spin icon-spinner icon-2x"></i><div className="datagrid-loading-message"></div></div></div>').appendTo(
            that.$container);
      }
      $loading.find('.datagrid-loading-message').text(
  ...
  DataGrid.prototype.getRowLayout = function (rowIndex) {
    ...
    return {
      height: rowHeight,
      top: layout.headerHeight + (rowIndex > 1 ? ((rowIndex - 1) * rowHeight)
          : 0) + rowIndex * layout.borderWidth
    };
  };
  ...
  DataGrid.prototype.updateLayout = function () {
    ...
    if (!layout.cols) {
      ...
      var colsLayout = [{
        left: 0,
        width: options.showRowIndex ? (rowIndexWidth === 'auto' ? ((dataLength
            + that.pager.skip + '').length * 8 + 18) : rowIndexWidth) : 0
      }];
    ...
    layout.height = layout.headerHeight + dataLength * (layout.rowHeight
        + layout.borderWidth);
    layout.spanMap = {};

    var containerHeight = options.height;
    if (containerHeight === 'page') {
      containerHeight = layout.headerHeight + that.pager.recPerPage
          * (layout.rowHeight + layout.borderWidth);
    }
  ...
  DataGrid.prototype.getCell = function (rowIndex, colIndex) {
    ...
    if (colIndex === 0) {
      ...
      type = 'head';
      value = config.label !== undefined ? config.label : (config.name
      !== undefined ? config.name : colIndex);
    } else {
      type = 'cell';
      value = config.data && config.data[that.options.dataItemIsArray ? colIndex
          : col.name];
    }
    if (rowIndex > 0) {
      var optionsValueOperator = that.options.valueOperator;
      var valueType = config.valueType;
      var valueOperator = config.valueOperator || (optionsValueOperator
      && valueType ? optionsValueOperator[valueType] : null);
    ...
    if (spanMap[config.id] || config.hidden) {
      cell.hidden = true;
    } else if ((config.colspan && config.colspan > 1) || (config.rowspan
        && config.rowspan > 1)) {
  ...
  DataGrid.prototype.getRowConfig = function (rowIndex) {
    ...
    if (!config) {
      config = $.extend({
            // height: 'auto'
            // fixed: false
          }, that.isFuncConfigs ? that.configs(rowId) : that.configs[rowId],
          that.userConfigs[rowId]);
    ...
    config.rowId = rowId !== undefined ? rowId : (rowIndex === 0 ? '#header'
        : rowIndex);
    return config;
  };
  ...
  DataGrid.prototype.checkRow = function (rowIndex, checked, holdEvents) {
    ...
    that.renderFixeds();
    if (!holdEvents) {
      that.$.callComEvent(that, 'onSelectRow',
          [rowId || 'all', checked, selections]);
    }
    return checked;
  };
  ...
  DataGrid.prototype.renderCell = function (rowIndex, colIndex, $row) {
    ...
    if (!$cell.length) {
      ...
      $cell = (options.cellCreator ? options.cellCreator(cell, that) : $(
          '<div class="datagrid-cell" />')).appendTo($row);
      $cell.attr({
        id: elementId,
        'data-type': cell.type,
      ...
      if (isCheckbox) {
        ...
        if (!$checkbox.length) {
          $checkbox = $(
              '<div class="checkbox-primary datagrid-checkbox"><label class="content"></label></div>').prependTo(
              $cell.addClass('datagrid-has-checkbox'));
        }
      }
    }
    ...
    var cellBoundsStyle = {
      ...
      width: borderWidth ? (config.width + ((colsLength - 1) === colIndex ? 2
          : 1) * borderWidth) : config.width,
      borderWidth: borderWidth
    };
    if (config.span) {
      if (config.rowspan && config.rowspan > 1) {
        cellBoundsStyle.bottom -= (config.rowspan - 1) * (layout.rowHeight
            + borderWidth);
      }
    ...
    if (colIndex > 0 && rowIndex === 0 && options.sortable && config.sort
        !== false) {
      var sorted = false;
      ...
      if (!$sorter.length) {
        $sorter = $(
            '<div class="datagrid-sorter"><i class="icon icon-sort"></i></div>').appendTo(
            $cell);
  ...
  DataGrid.prototype.renderRow = function (rowIndex) {
    ...
    var $row = $('#' + elementId);
    if (!$row.length) {
      $row = (options.rowCreator ? options.rowCreator(rowIndex, that) : $(
          '<div class="datagrid-row" />')).appendTo(that.$cells);
      $row.attr({
        id: elementId,
        'data-row': rowIndex,
        'data-id': rowIndex
      }).css({
        top: layout.partialRendering ? (rowLayout.top - layout.scrollTop)
            : rowLayout.top,
  ...
  DataGrid.prototype.renderData = function () {
    ...
    if (layout.partialRendering) {
      ...
      startRenderRow = Math.min(endRenderRow, Math.max(1,
          Math.floor((layout.scrollTop - layout.headerHeight) / rowHeight)));
      endRenderRow = Math.min(endRenderRow, Math.max(1, Math.ceil(
          (layout.scrollTop + layout.containerHeight - layout.headerHeight)
          / rowHeight)));
      that.$cells.find('.datagrid-row').each(function () {
        var $row = $(this);
        var rowIndex = $row.data('row');
        if (rowIndex > 0 && !$row.hasClass('datagrid-fixed') && (rowIndex
            < startRenderRow || rowIndex > endRenderRow)) {
          $row.remove();
        }
      });
    }
    ...
    if (layout.vScrollSpare) {
      ...
      if (typeof fixedTopUntil === 'number' && fixedTopUntil > 0
          && fixedTopUntil < startRenderRow) {
        for (var i = 1; i <= fixedTopUntil; ++i) {
          that.renderRow(i);
        }
      }
      if (typeof fixedBottomFrom === 'number' && fixedBottomFrom > 0
          && fixedBottomFrom > endRenderRow) {
  ...
  DataGrid.prototype.render = function (ignoreDelay) {
    ...
    that.loadData(function (data) {
      ...
      that.$cells.css({
        width: layout.width,
        height: layout.partialRendering ? layout.containerHeight
            : that.layout.height
      });
  ...
  DataGrid.prototype.renderScrolls = function () {
    ...
    layout.scrollLeft = showHBar ? Math.max(0,
        Math.min(hSize, layout.scrollLeft)) : 0;
    layout.scrollTop = showVBar ? Math.max(0, Math.min(vSize, layout.scrollTop))
        : 0;
    if (showVBar) {
  ...
  DataGrid.prototype.scroll = function (scrollLeft, scrollTop, ignoreDelay) {
    ...
    if (scrollDelay) {
      if (!ignoreDelay && that.lastScrollTime && (now - that.lastScrollTime)
          < scrollDelay) {
    ...
    that.$.callComEvent(that, 'onScroll',
        [scrollLeft, scrollTop, {vScrolled: vScrolled, hScrolled: hScrolled}]);
  };
  ...
  DataGrid.prototype.renderFixeds = function () {
    ...
    that.$cells.find('.datagrid-fixed-edge-top').removeClass(
        'datagrid-fixed-edge-top');
    that.$cells.find('.datagrid-fixed-edge-bottom').removeClass(
        'datagrid-fixed-edge-bottom');
    that.$cells.find('.datagrid-fixed-edge-left').removeClass(
        'datagrid-fixed-edge-left');
    that.$cells.find('.datagrid-fixed-edge-right').removeClass(
    ...
    if (layout.vScrollSpare) {
      ...
      if (typeof fixedTopUntil === 'number' && fixedTopUntil > -1) {
        ...
        for (var i = 0; i <= fixedTopUntil; ++i) {
          ...
          var fixedTop = layout.partialRendering ? rowLayout.top
              : (rowLayout.top + layout.scrollTop);
      ...
      if (typeof fixedBottomFrom === 'number' && fixedBottomFrom > -1) {
        fixedBottomFrom = Math.max(fixedTopUntil > -1 ? (fixedTopUntil + 1) : 1,
            Math.min(fixedBottomFrom, layout.rowsLength));
        for (var i = fixedBottomFrom; i < layout.rowsLength; ++i) {
          var rowLayout = that.getRowLayout(i);
          var $row = $('#' + that.id + '-row-' + i);
          var fixedTop = layout.partialRendering ? (rowLayout.top
              - layout.vScrollSpare) : (rowLayout.top - layout.vScrollSpare
              + layout.scrollTop);
          $row.addClass('datagrid-fixed').css('top', fixedTop);
    ...
    if (layout.hScrollSpare) {
      ...
      if (typeof fixedLeftUntil === 'number' && fixedLeftUntil > -1) {
        ...
        for (var i = 0; i <= fixedLeftUntil; ++i) {
          ...
          var fixedLeft = colLayout.left + layout.scrollLeft
              - layout.borderWidth;
      ...
      if (typeof fixedRightFrom === 'number' && fixedRightFrom > -1) {
        fixedRightFrom = Math.max(
            fixedLeftUntil > -1 ? (fixedLeftUntil + 1) : 1,
            Math.min(fixedRightFrom, layout.colsLength));
        for (var i = fixedRightFrom; i < layout.colsLength; ++i) {
          var colLayout = layout.cols[i];
          var $cols = that.$cells.find('.datagrid-cell[data-col="' + i + '"]');
          var fixedLeft = colLayout.left - layout.hScrollSpare
              + layout.scrollLeft;
          $cols.addClass('datagrid-fixed').css('left', fixedLeft);
  ...
  $.fn.datagrid = function (option) {
    return this.each(function () {
      ...
      if (!data) {
        $this.data(NAME, (data = new DataGrid(this, options)));
      }
