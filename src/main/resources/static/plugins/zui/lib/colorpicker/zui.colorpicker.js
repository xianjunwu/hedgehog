...
(function ($) {
  ...
  ColorPicker.DEFAULTS = {
    colors: ['#00BCD4', '#388E3C', '#3280fc', '#3F51B5', '#9C27B0', '#795548',
      '#F57C00', '#F44336', '#E91E63'],
    pullMenuRight: true,
    wrapper: 'btn-wrapper',
    tileSize: 30,
    lineCount: 5,
  ...
  ColorPicker.prototype.init = function () {
    ...
    that.$picker.addClass(options.wrapper).find('.cp-title').toggle(
        options.title !== undefined).text(options.title);
    that.$menu = that.$picker.find('.dropdown-menu').toggleClass('pull-right',
        options.pullMenuRight);
    that.$btn = that.$picker.find('.btn.dropdown-toggle');
    that.$btn.find('.ic').addClass('icon-' + options.icon);
    if (options.btnTip) {
      that.$picker.attr('data-toggle', 'tooltip').tooltip({
  ...
  ColorPicker.prototype.addColor = function (color) {
    ...
    this.$menu.append($('<li/>').css(
        {width: options.tileSize, height: options.tileSize}).append($a));
    if (options.optional) {
  ...
  ColorPicker.prototype.updateColors = function (colors) {
    ...
    if (options.optional) {
      var $li = $('<li><a class="cp-tile empty" href="###"></a></li>').css(
          {width: options.tileSize, height: options.tileSize});
      this.$menu.append($li);
      bestLineCount++;
    }
    $menu.css('width',
        Math.min(bestLineCount, options.lineCount) * options.tileSize + 6);
  };
